...
void uci_identify() {
    std::cout << "id name " << ENGINE_NAME << ' ' << ENGINE_VERSION << '\n'
              ...
              // Phase 3 Step 3: TEMPORARY SPRT A/B toggle (phase3.md §3 decision 4).
              // Removed in the sign-off commit once the Step 3 test reaches PASS.
...
void uci_go(Position& pos, TranspositionTable& tt, std::istringstream& iss,
            const std::vector<std::uint64_t>& gameKeys, bool optQsearch, bool optCheckExt) {
    ...
    info.use_check_ext = optCheckExt;   // Phase 3 Step 3: A/B knob (UCI option `CheckExtension`)
...
void uci_loop() {
    ...
    bool optCheckExt = true;  // Phase 3 Step 3: `CheckExtension` UCI option (temporary SPRT toggle)
    ...
    while (std::getline(std::cin, line)) {
        ...
        if (token == "uci") {
            ...
            } else if (name == "CheckExtension") {
                optCheckExt = (value == "true" || value == "1");
            }
        } else if (token == "position") {
            uci_position(pos, iss, gameKeys);
        } else if (token == "go") {
            uci_go(pos, tt, iss, gameKeys, optQsearch, optCheckExt);
        } else if (token == "bench") {
