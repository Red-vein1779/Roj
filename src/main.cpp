...
void uci_identify() {
    std::cout << "id name " << ENGINE_NAME << ' ' << ENGINE_VERSION << '\n'
              ...
              // Phase 3 Step 4: TEMPORARY SPRT A/B toggle (phase3.md §3 decision 4).
              // Removed in the sign-off commit once the NMP SPRT reaches a decision.
...
void uci_go(Position& pos, TranspositionTable& tt, std::istringstream& iss,
            const std::vector<std::uint64_t>& gameKeys, bool optQsearch, bool optNullMove) {
    ...
    info.use_nullmove = optNullMove;  // Phase 3 Step 4: A/B knob (UCI option `NullMove`)
...
void uci_loop() {
    ...
    bool optNullMove = true;  // Phase 3 Step 4: `NullMove` UCI option (temporary SPRT toggle)
    ...
    while (std::getline(std::cin, line)) {
        ...
        if (token == "uci") {
            ...
            if (name == "Hash") {
                ...
                optNullMove = (value == "true" || value == "1");
            }
        } else if (token == "position") {
            uci_position(pos, iss, gameKeys);
        } else if (token == "go") {
            uci_go(pos, tt, iss, gameKeys, optQsearch, optNullMove);
        } else if (token == "bench") {
