...
void uci_identify() {
    std::cout << "id name " << ENGINE_NAME << ' ' << ENGINE_VERSION << '\n'
              ...
              // Phase 3 Step 7: TEMPORARY SPRT A/B toggle (phase3.md §3 decision 4).
              // Removed in the sign-off commit once the Step 7 SPRT reaches a decision.
              << "option name SEEQPrune type check default true\n"
              << "uciok" << std::endl;
}
...
void uci_go(Position& pos, TranspositionTable& tt, std::istringstream& iss,
            const std::vector<std::uint64_t>& gameKeys, bool optQsearch, bool optSeeQ) {
    ...
    info.use_seeq_prune = optSeeQ;   // Phase 3 Step 7: A/B knob (UCI option `SEEQPrune`)
...
void uci_loop() {
    ...
    bool optSeeQ    = true;   // Phase 3 Step 7: `SEEQPrune` UCI option (temporary SPRT toggle)
    ...
    while (std::getline(std::cin, line)) {
        ...
        if (token == "uci") {
            ...
            } else if (name == "SEEQPrune") {
                optSeeQ = (value == "true" || value == "1");
            }
        } else if (token == "position") {
            uci_position(pos, iss, gameKeys);
        } else if (token == "go") {
            uci_go(pos, tt, iss, gameKeys, optQsearch, optSeeQ);
        } else if (token == "bench") {
